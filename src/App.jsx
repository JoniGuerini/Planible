...
const getBaseProductionForLine = (lineId) => PRODUCTION_LINES.find(l => l.id === lineId)?.baseProduction ?? 1
...
function App() {
  ...
  const [essence, setEssence] = useState(() => initial?.essence ?? new Decimal(0))
  ...
  const universalProductionRatePerSec = PRODUCTION_LINES.reduce((acc, line) => {
    ...
    const baseProd = getBaseProductionForLine(line.id)
  ...
  const atomsPerSecond = 1 + PRODUCTION_LINES.reduce((sum, line) => sum + (lineData[line.id]?.atomosClaimed ?? 0), 0)

  const resetGame = () => {
    clearSaveCache()
    localStorage.removeItem(SAVE_KEY)
    setInsights(new Decimal(0))
    setEssence(new Decimal(0))
    const fresh = {}
    for (const line of PRODUCTION_LINES) {
  ...
  const computeOfflineProduction = (lineDataSnapshot, durationMs) => {
    ...
    for (const line of PRODUCTION_LINES) {
      ...
      const getEffectiveCycleTime = (gen) => {
        ...
        return rank === 0 ? base : Math.max(MIN_CYCLE_MS_OFFLINE, base / Math.pow(2, rank))
      }
      ...
      const withProgress = gens.map((gen) => {
        ...
        const cycles = Math.floor(cap / getEffectiveCycleTime(gen))
        return { ...gen, _cycles: cycles }
      })
      ...
      const baseProd = getBaseProductionForLine(line.id)
      const currencyFromGen1 = new Decimal(cycles1).times(count1).times(mult1).times(baseProd)
    ...
    const atomsPerSecond = 1 + atomosBonus
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      while (elapsed >= ATOM_CYCLE_MS) {
        essenceRef.current = (essenceRef.current || new Decimal(0)).plus(atomsPerSecond)
  ...
  const claimAtomosUpgrade = (lineId) => {
    const ld = lineData[lineId]
    if (!ld) return
    const totalProduced = ld.totalProduced instanceof Decimal ? ld.totalProduced : new Decimal(ld.totalProduced ?? 0)
    const atomosClaimed = ld.atomosClaimed ?? 0
    const nextMilestone = getMilestone(atomosClaimed)
    if (!totalProduced.gte(nextMilestone)) return
    setLineData(prev => ({
      ...prev,
      [lineId]: {
        ...prev[lineId],
  ...
  const getClaimableAtomosTotal = () => {
    ...
    for (const line of PRODUCTION_LINES) {
      ...
      let claimed = 0
      while (totalProduced.gte(getMilestone(idx))) {
        claimed++
        idx++
      }
      total += claimed
              ...
              {tab !== 'atomos' && (
                <div className="flex flex-wrap gap-2 overflow-visible">
                  {PRODUCTION_LINES.map((line) => {
                    ...
                    return (
                      <div key={line.id} className="relative min-w-[9rem] max-w-[12rem] flex-1 shrink-0 overflow-visible">
                        <button
                          type="button"
                        ...
                        <>
                          ...
                          <span className="relative z-10 flex items-center gap-2 text-xs text-white/90 [text-shadow:0_1px_2px_rgba(0,0,0,0.9),0_0_6px_rgba(0,0,0,0.8)]">
                            <span className="flex items-center gap-1">
                              ...
                              <span className={cn("tabular-nums font-medium", !currency.gte(currentCost) && "text-red-400", currency.gte(currentCost) && canBuyG1 && "text-emerald-400")}>{formatCost(currentCost)}</span>
                            </span>
                            <span className="flex items-center gap-1">
                              <Atom className="w-4 h-4 shrink-0 text-cyan-300/90" />
                              <span className={cn("tabular-nums font-medium", !ess.gte(essenceRequired) && "text-red-400", ess.gte(essenceRequired) && canBuyG1 && "text-emerald-400")}>{essenceRequired}</span>
                            </span>
                        ...
                        <>
                            ...
                            >
                              ...
                              <span className="text-xs font-medium text-white/95 truncate w-[7.5rem] shrink-0 text-right">
                                {(() => {
                                  ...
                                  const baseProd = gen.id === 1 ? getBaseProductionForLine(activeProductionLine) : 1
                          ...
                          {isAutomated ? (
                            ...
                            <button
                              ...
                              title={`${formatCost(currentCost)} + ${essenceRequired} Átomo`}
                            >
                              <div className="absolute inset-0 flex flex-col pointer-events-none">
                                {canAfford ? (
                                  <>
                              ...
                              <div className="relative z-10 flex items-center gap-1.5 truncate">
                                <Coins className="w-4 h-4 shrink-0 text-amber-300/90" />
                                <span className={cn("tabular-nums truncate", !currency.gte(currentCost) && "text-red-400")}>
                                  {formatCost(currentCost)}
                                </span>
                                <span className="text-white/60">+</span>
                                <Atom className="w-4 h-4 shrink-0 text-cyan-300/90" />
                                <span className={cn("tabular-nums truncate", !ess.gte(essenceRequired) && "text-red-400")}>
                                  {essenceRequired}
                                </span>
                              </div>
                            </button>
              ...
              <div className="flex flex-col gap-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  {PRODUCTION_LINES.map((line) => {
                    ...
                    const atomosClaimed = ld?.atomosClaimed ?? 0
                    const nextMilestone = getMilestone(atomosClaimed)
                    const canClaim = totalProduced.gte(nextMilestone)
                    const progress = nextMilestone.gt(0) ? Math.min(100, totalProduced.div(nextMilestone).times(100).toNumber()) : 100
                    const LineIcon = line.icon
                    return (
                      <button
                        key={line.id}
                        ...
                        <div className="relative z-10 p-4 flex flex-col gap-2">
                          ...
                          {canClaim ? (
                            ...
                            <span className="text-xs text-white/60 [text-shadow:0_1px_2px_rgba(0,0,0,0.9)]">
                              {format(totalProduced)} / {format(nextMilestone)}
                            </span>
                          )}
                        </div>
                      </button>
